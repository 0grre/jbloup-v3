...
// src/App.vue
var App_default = "./App-e4061eff9b78c27d.vue";
...
var isESModule = function(obj) {
  return obj.__esModule || obj[Symbol.toStringTag] === "Module";
};
var applyToParams = function(fn, params) {
  const newParams = {};
...
var commonEncode = function(text) {
  return encodeURI("" + text).replace(ENC_PIPE_RE, "|").replace(ENC_BRACKET_OPEN_RE, "[").replace(ENC_BRACKET_CLOSE_RE, "]");
};
var encodeHash = function(text) {
  return commonEncode(text).replace(ENC_CURLY_OPEN_RE, "{").replace(ENC_CURLY_CLOSE_RE, "}").replace(ENC_CARET_RE, "^");
};
var encodeQueryValue = function(text) {
  return commonEncode(text).replace(PLUS_RE, "%2B").replace(ENC_SPACE_RE, "+").replace(HASH_RE, "%23").replace(AMPERSAND_RE, "%26").replace(ENC_BACKTICK_RE, "`").replace(ENC_CURLY_OPEN_RE, "{").replace(ENC_CURLY_CLOSE_RE, "}").replace(ENC_CARET_RE, "^");
};
var encodeQueryKey = function(text) {
  return encodeQueryValue(text).replace(EQUAL_RE, "%3D");
};
var encodePath = function(text) {
  return commonEncode(text).replace(HASH_RE, "%23").replace(IM_RE, "%3F");
};
var encodeParam = function(text) {
...
var parseURL = function(parseQuery, location2, currentLocation = "/") {
...
var stringifyURL = function(stringifyQuery, location2) {
  const query = location2.query ? stringifyQuery(location2.query) : "";
  return location2.path + (query && "?") + query + (location2.hash || "");
};
var stripBase = function(pathname, base) {
  if (!base || !pathname.toLowerCase().startsWith(base.toLowerCase()))
    return pathname;
  return pathname.slice(base.length) || "/";
};
var isSameRouteLocation = function(stringifyQuery, a, b) {
  const aLastIndex = a.matched.length - 1;
  const bLastIndex = b.matched.length - 1;
  return aLastIndex > -1 && aLastIndex === bLastIndex && isSameRouteRecord(a.matched[aLastIndex], b.matched[bLastIndex]) && isSameRouteLocationParams(a.params, b.params) && stringifyQuery(a.query) === stringifyQuery(b.query) && a.hash === b.hash;
};
var isSameRouteRecord = function(a, b) {
  return (a.aliasOf || a) === (b.aliasOf || b);
};
var isSameRouteLocationParams = function(a, b) {
...
var isSameRouteLocationParamsValue = function(a, b) {
  return isArray2(a) ? isEquivalentArray(a, b) : isArray2(b) ? isEquivalentArray(b, a) : a === b;
};
var isEquivalentArray = function(a, b) {
...
var normalizeBase = function(base) {
  if (!base) {
    if (isBrowser) {
...
var createHref = function(base, location2) {
  return base.replace(BEFORE_HASH_RE, "#") + location2;
};
var getElementPosition = function(el, offset) {
...
var getScrollKey = function(path, delta) {
  const position = history.state ? history.state.position - delta : -1;
  return position + path;
};
var saveScrollPosition = function(key, scrollPosition) {
  scrollPositions.set(key, scrollPosition);
};
var getSavedScrollPosition = function(key) {
  const scroll = scrollPositions.get(key);
  scrollPositions.delete(key);
  return scroll;
};
var createCurrentLocation = function(base, location2) {
...
var useHistoryListeners = function(base, historyState, currentLocation, replace) {
  let listeners = [];
  let teardowns = [];
  let pauseState = null;
...
var buildState = function(back, current, forward, replaced = false, computeScroll = false) {
  return {
    back,
    current,
    forward,
...
var useHistoryStateNavigation = function(base) {
...
var createWebHistory = function(base) {
...
var isRouteLocation = function(route) {
  return typeof route === "string" || route && typeof route === "object";
};
var isRouteName = function(name) {
  return typeof name === "string" || typeof name === "symbol";
};
var createRouterError = function(type, params) {
  if (true) {
...
var isNavigationFailure = function(error, type) {
  return error instanceof Error && NavigationFailureSymbol in error && (type == null || !!(error.type & type));
};
var stringifyRoute = function(to) {
  if (typeof to === "string")
    return to;
  if (to.path != null)
    return to.path;
...
var tokensToParser = function(segments, extraOptions) {
...
var compareScoreArray = function(a, b) {
  let i = 0;
...
var comparePathParserScore = function(a, b) {
  let i = 0;
...
var isLastScoreNegative = function(score) {
...
var createRouteRecordMatcher = function(record, parent, options) {
...
var createRouterMatcher = function(routes, globalOptions) {
  const matchers = [];
...
var paramsFromLocation = function(params, keys) {
  const newParams = {};
...
var normalizeRouteRecord = function(record) {
  return {
    path: record.path,
...
var normalizeRecordProps = function(record) {
  const propsObject = {};
...
var isAliasRecord = function(record) {
  while (record) {
...
var mergeMetaFields = function(matched) {
  return matched.reduce((meta, record) => assign(meta, record.meta), {});
};
var mergeOptions2 = function(defaults, partialOptions) {
  const options = {};
...
var isSameParam = function(a, b) {
  return a.name === b.name && a.optional === b.optional && a.repeatable === b.repeatable;
};
var checkSameParams = function(a, b) {
...
var checkChildMissingNameWithEmptyPath = function(mainNormalizedRecord, parent) {
...
var checkMissingParamsInAbsolutePath = function(record, parent) {
...
var isRecordChildOf = function(record, parent) {
...
var guardToPromiseFn = function(guard, to, from, record, name, runWithContext = (fn) => fn()) {
...
var canOnlyBeCalledOnce = function(next, to, from) {
  let called = 0;
  return function() {
    if (called++ === 1)
...
var extractComponentsGuards = function(matched, guardType, to, from, runWithContext = (fn) => fn()) {
  const guards = [];
...
var isRouteComponent = function(component) {
  return typeof component === "object" || "displayName" in component || "props" in component || "__vccOpts" in component;
};
var useLink = function(props) {
...
var includesParams = function(outer, inner) {
...
var getOriginalPath = function(record) {
  return record ? record.aliasOf ? record.aliasOf.path : record.path : "";
};
var normalizeSlot2 = function(slot, data) {
  if (!slot)
    return null;
  const slotContent = slot(data);
  return slotContent.length === 1 ? slotContent[0] : slotContent;
};
var warnDeprecatedUsage = function() {
  const instance = getCurrentInstance();
...
var formatRouteLocation = function(routeLocation, tooltip) {
...
var formatDisplay = function(display) {
  return {
    _custom: {
      display
    }
...
var addDevtools = function(app, router, matcher) {
...
var modifierForKey = function(key) {
  if (key.optional) {
...
var formatRouteRecordMatcherForStateInspector = function(route) {
  const { record } = route;
  const fields = [
...
var formatRouteRecordForInspector = function(route) {
  const tags = [];
  const { record } = route;
  if (record.name != null) {
    tags.push({
...
var markRouteRecordActive = function(route, currentRoute) {
...
var resetMatchStateOnRouteRecord = function(route) {
  route.__vd_match = false;
  route.children.forEach(resetMatchStateOnRouteRecord);
};
var isRouteMatching = function(route, filter) {
...
var extractChangingRecords = function(to, from) {
...
// src/pages/Home.vue
var Home_default = "./Home-b837c4918d14d58a.vue";

// src/pages/Archives.vue
var Archives_default = "./Archives-29b9ffd357b0818a.vue";

// src/router/index.js
var routes = [
  { path: "/", component: Home_default },
  { path: "/archives", component: Archives_default }
];
...
// src/main.js
createApp(App_default).use(router_default).mount("#app");
